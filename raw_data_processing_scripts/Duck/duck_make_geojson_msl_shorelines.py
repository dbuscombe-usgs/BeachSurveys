...
sitename = 'DUCK'
fp_raw = r"/media/marda/FOURTB/SDS/Zoo_SDS_paper/field_survey_data/Duck/duck_insitu/raw/"
...
filenames = [_ for _ in filenames if not _ == 'ERA5_waves_DUCK']

# skip problematic survey
filenames = sorted([f for f in filenames if '19890523' not in f])


for fn in filenames:

    data = xr.open_dataset(fp_raw+fn,engine='netcdf4')
    date_str = fn.split('_')[-1].split('.nc')[0]
    elevation = np.array(data.variables['elevation'][:])
    northing = np.array(data.variables['northing'][:])
    easting = np.array(data.variables['easting'][:])

    x = np.arange(np.min(easting),np.max(easting),1)
    y = np.arange(np.min(northing),np.max(northing),1)
    xv, yv = np.meshgrid(x,y, indexing='xy')
    grid_z0 = griddata((easting,northing), elevation, (xv, yv), method='linear')

    try:

        cs=plt.contour(xv,yv,grid_z0,(-99,MHWS,99), colors='k')
        plt.close()
        ...
        gdf = gpd.GeoDataFrame(
            df, geometry=gpd.points_from_xy(df.x, df.y), crs="EPSG:32119" #crs="EPSG:26911"
        )

        gdf2 = gdf.groupby(['z'])['geometry'].apply(lambda x: LineString(x.tolist()))
        gdf2 = gpd.GeoDataFrame(gdf2,  crs="EPSG:32119")
        gdf2.to_file(f"{date_str}_lineMHWS.json", driver="GeoJSON")
        ...
        gdf2 = gdf.groupby(['z'])['geometry'].apply(lambda x: LineString(x.tolist()))
        gdf2 = gpd.GeoDataFrame(gdf2,  crs="EPSG:32119")
        gdf2.to_file(f"{date_str}_lineMSL.json", driver="GeoJSON")
...
transects_dir = r"/media/marda/FOURTB/SDS/Zoo_SDS_paper/field_survey_data/Duck/DuckforZooSDS_paper_analysis/"
shoreline_dir = r"/media/marda/FOURTB/SDS/Zoo_SDS_paper/field_survey_data/Duck/DuckforZooSDS_paper_analysis/msl"

##############################################################
transect_files = glob(transects_dir + "/*TR*.geojson")
...
##############################################################
shoreline_files = glob(shoreline_dir + "/*MSL.json")
...
plt.subplot(312)
id = np.unique(joined_df.transect_id.values)[40]
y = joined_df[joined_df.transect_id.values==id]['cross_distance']
x = joined_df[joined_df.transect_id.values==id]['dates']
plt.plot(x,y,'k-', alpha=0.5)
plt.plot(x,y,'k.')
...
plt.subplot(313)
id = np.unique(joined_df.transect_id.values)[70]
y = joined_df[joined_df.transect_id.values==id]['cross_distance']
x = joined_df[joined_df.transect_id.values==id]['dates']
plt.plot(x,y,'k-', alpha=0.5)
plt.plot(x,y,'k.')
...
# plt.show()
plt.savefig('Duck_MSL_timeseries_example.png', dpi=300, bbox_inches='tight')
plt.close()


joined_df.to_csv('Duck_MSL_fieldsurvey_shoreline_chainage.csv')

joined_gdf = gpd.GeoDataFrame(
    joined_df, geometry=gpd.points_from_xy(joined_df.x, joined_df.y), crs="EPSG:4326"
)

joined_gdf.to_file("Duck_MSL_fieldsurvey_shoreline_chainage.geojson", driver="GeoJSON") 
...
shoreline_dir = r"/media/marda/FOURTB/SDS/Zoo_SDS_paper/field_survey_data/Duck/DuckforZooSDS_paper_analysis/mhws"

shoreline_files = glob(shoreline_dir + "/*MHWS.json")
if not shoreline_files:
    raise ValueError(f"No shoreline files found in the directory {shoreline_dir}")

# this scripts assumes all transects are epsg 4326
transect_files = glob(transects_dir + "/T*.geojson")
...
# plt.show()
plt.savefig('Duck_MHWStimeseries_example.png', dpi=300, bbox_inches='tight')
plt.close()



joined_df.to_csv('Duck_MHWS_fieldsurvey_shoreline_chainage.csv')

joined_gdf = gpd.GeoDataFrame(
    joined_df, geometry=gpd.points_from_xy(joined_df.x, joined_df.y), crs="EPSG:4326"
)

joined_gdf.to_file("Duck_MHWS_fieldsurvey_shoreline_chainage.geojson", driver="GeoJSON") 
...
#################################################################################
### SLOPE
joined_gdf_MSL = gpd.read_file("Duck_MSL_fieldsurvey_shoreline_chainage.geojson")
joined_gdf_MHW = gpd.read_file("Duck_MHWS_fieldsurvey_shoreline_chainage.geojson")
...
msl_dates = np.unique(joined_gdf_MSL['dates']) #874 dates
mhw_dates = np.unique(joined_gdf_MHW['dates']) #871 dates

comb_dates = [d for d in mhw_dates if d in msl_dates] #870
...
shapes = (joined_gdf_MSL_2.shape[0], joined_gdf_MHW_2.shape[0])
ind = np.argmin(shapes)

joined_gdf_MSL_2 = joined_gdf_MSL_2[:shapes[ind]]
joined_gdf_MHW_2 = joined_gdf_MHW_2[:shapes[ind]]
...
df = pd.DataFrame(data=slopes,  index=np.unique(joined_gdf_MHW.dates.values)[:-1], columns=np.unique(joined_gdf_MSL['transect_id']))

df.to_csv('Duck_fieldsurvey_slopes.csv')


plt.figure(figsize=(16,8))

plt.subplot(121)
plt.plot(np.linspace(0,81*50,81), df.mean(axis=0), label='mean')
plt.plot(np.linspace(0,81*50,81), df.median(axis=0), label='median')
...
plt.savefig('Duck_slope_space_time.png',dpi=200, bbox_inches='tight')
plt.close()
...
plt.savefig('Duck_Avslope_MonthOfYear.png',dpi=200, bbox_inches='tight')
plt.close()
...
plt.subplot(121)
plt.pcolormesh(np.arange(0,12), np.arange(60,81), np.vstack(MM)[60:,:], vmin=0.04, vmax=0.1)
plt.colorbar(extend='both')
plt.xlabel('Month of Year')
plt.ylabel('Transect ID')
plt.gca().invert_yaxis()
plt.savefig('Duck_spacetime_monthly.png',dpi=200, bbox_inches='tight')
plt.close()
...
# transect_files = glob(transects_dir + "/*TR_ID*.geojson")
...
# transects_gdf.to_crs(32119, inplace=True)
# print(transects_gdf.crs)
...
# shoreline_files = sorted(glob(shoreline_dir + "/*lineMSL.json"))
...
# df.to_csv('Duck_fieldsurvey_shoreline_chainage_v2.csv')
...
# dz = contour_dict[sitename]['MHWS'] - contour_dict[sitename]['MSL']
