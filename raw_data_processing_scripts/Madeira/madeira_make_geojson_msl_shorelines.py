...
infolder = '/media/marda/FOURTB/SDS/Zoo_SDS_paper/field_survey_data/Madeira/data'

list_subfolders_with_paths = [f.path for f in os.scandir(infolder) if f.is_dir()]

dates = []
data=[]
for counter,folder in enumerate(list_subfolders_with_paths):
    files = glob(folder+os.sep+'*.xyz')
    X = []; Y = []; Z = []
    for f in files:
    ...
    data.append(grid_z0)
    dates.append(folder.split(os.sep)[-1].split('_')[0])

data = np.dstack(data)

# dates = [str(i) for i in np.arange(data.shape[-1])]

shoreline_contour = 1.156 #MHWS
...
for k in range(data.shape[-1]):
    ...
    try:
        cs=plt.contour(xv,yv,data[:,:,k],(-99,shoreline_contour,99), colors='k')
        ...
        gdf = gpd.GeoDataFrame(
            df, geometry=gpd.points_from_xy(df.x, df.y), crs="EPSG:32617"
        )
        # gdf.to_file(f"{dates[k]}.json", driver="GeoJSON")

        gdf2 = gdf.groupby(['z'])['geometry'].apply(lambda x: LineString(x.tolist()))
        gdf2 = gpd.GeoDataFrame(gdf2,  crs="EPSG:32617")
        # gdf2.to_file(f"{dates[k]}_line.json", driver="GeoJSON")
        gdf2.to_file(f"{dates[k]}_lineMHWS.json", driver="GeoJSON")

        # plt.imshow(data[:,:,k], extent=(np.min(x),np.max(x),np.min(y),np.max(y)), origin='lower',cmap=cut_terrain_map, norm=norm)
        # cs=plt.contour(xv,yv,data[:,:,k],(-99,shoreline_contour,99), colors='k')

        # if k>0:
        #     plt.contour(xv,yv,data[:,:,k-1],(-99,shoreline_contour,99), colors='r')

        # plt.title(dates[k],loc='left')
        # plt.axis('off')
        # plt.savefig(f'{k}.png',dpi=200,bbox_inches='tight')
        # plt.close()
    except:
        pass


shoreline_contour = -0.096 #MSL
plt.close('all')
Xmsl = []
Ymsl = []
survey = []
...
################################################################
transects_dir = r"/media/marda/FOURTB/SDS/Zoo_SDS_paper/field_survey_data/Madeira/MadeiraforZooSDS_paper_analysis/"
shoreline_dir = r"/media/marda/FOURTB/SDS/Zoo_SDS_paper/field_survey_data/Madeira/MadeiraforZooSDS_paper_analysis/msl"
...
plt.subplot(312)
id = np.unique(joined_df.transect_id.values)[6]
y = joined_df[joined_df.transect_id.values==id]['cross_distance']
x = joined_df[joined_df.transect_id.values==id]['dates']
plt.plot(x,y,'k-', alpha=0.5)
plt.plot(x,y,'k.')
...
# plt.show()
plt.savefig('Madeira_MSL_timeseries_example.png', dpi=300, bbox_inches='tight')
plt.close()


joined_df.to_csv('Madeira_MSL_fieldsurvey_shoreline_chainage.csv')

joined_gdf = gpd.GeoDataFrame(
    joined_df, geometry=gpd.points_from_xy(joined_df.x, joined_df.y), crs="EPSG:4326"
)

joined_gdf.to_file("Madeira_MSL_fieldsurvey_shoreline_chainage.geojson", driver="GeoJSON") 
...
shoreline_dir = r"/media/marda/FOURTB/SDS/Zoo_SDS_paper/field_survey_data/Madeira/MadeiraforZooSDS_paper_analysis/mhws"
...
# plt.show()
plt.savefig('Madeira_MHWStimeseries_example.png', dpi=300, bbox_inches='tight')
plt.close()



joined_df.to_csv('Madeira_MHWS_fieldsurvey_shoreline_chainage.csv')

joined_gdf = gpd.GeoDataFrame(
    joined_df, geometry=gpd.points_from_xy(joined_df.x, joined_df.y), crs="EPSG:4326"
)

joined_gdf.to_file("Madeira_MHWS_fieldsurvey_shoreline_chainage.geojson", driver="GeoJSON") 



#################################################################################
### SLOPE
joined_gdf_MSL = gpd.read_file("Madeira_MSL_fieldsurvey_shoreline_chainage.geojson")
joined_gdf_MHW = gpd.read_file("Madeira_MHWS_fieldsurvey_shoreline_chainage.geojson")
...
df.to_csv('Madeira_fieldsurvey_slopes.csv')


plt.figure(figsize=(16,8))

plt.subplot(121)
plt.plot(np.linspace(0,545,13), df.mean(axis=0), label='mean')
plt.plot(np.linspace(0,545,13), df.median(axis=0), label='median')
...
plt.savefig('Madeira_slope_space_time.png',dpi=200, bbox_inches='tight')
plt.close()
...
plt.savefig('Madeira_Avslope_MonthOfYear.png',dpi=200, bbox_inches='tight')
plt.close()
...
plt.subplot(121)
plt.pcolormesh(np.arange(0,12), np.arange(0,13), np.vstack(MM))#, vmin=0.04, vmax=0.1)
plt.colorbar(extend='both')
plt.xlabel('Month of Year')
plt.ylabel('Transect ID')
plt.gca().invert_yaxis()
plt.savefig('Madeira_spacetime_monthly.png',dpi=200, bbox_inches='tight')
plt.close()
...
# transects_gdf.to_crs(32617, inplace=True)
# print(transects_gdf.crs)
...
# shoreline_dir = r"/media/marda/FOURTB/SDS/Zoo_SDS_paper/field_survey_data/Madeira/"
# shoreline_files = glob(shoreline_dir + "/*line.json")
...
# df.to_csv('Madeira_fieldsurvey_shoreline_chainage.csv')
...
# vslope = (np.vstack(v) - np.vstack(v2))*2



# dz = (1.156 + 0.096)/2
# slopes = np.abs(np.vstack(np.tan(dz/vslope)))

# slopes[slopes>.2]=np.nan
# slopes[slopes<0.001]=np.nan
# slopes[slopes>.2]=np.nan


# df = pd.DataFrame(data=slopes,  index=cross_dist.keys(), columns=dates)
# df.to_csv('MadeiraBeach_fieldsurvey_slopes.csv')
