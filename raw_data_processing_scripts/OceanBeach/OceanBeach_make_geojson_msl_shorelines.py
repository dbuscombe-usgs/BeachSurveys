...
## script to find intersections of in situ shorelines on transects
...
def combine_geojson_files_with_dates(file_paths):
    ...
    for file in file_paths:
        ...
        gdf["date"] = datetime.strptime('_'.join(os.path.basename(file).split(".")[0].split('_')[:3]), "%m_%d_%Y")
...
def combine_geojson_files_with_dates2(file_paths):
    """
    Combines multiple GeoJSON files into a single GeoDataFrame and assigns a date column 
    based on the filenames.
    ...
    for file in file_paths:
        ...
        if "z" in gdf.columns:
                gdf.drop(columns="z", inplace=True)
        # date_str = os.path.basename(file).split("_")[0]
        gdf["date"] = datetime.strptime('_'.join(os.path.basename(file).split(".")[0].split('_')[:3]), "%Y_%m_%d")
...
############################################################
transects_dir = r"/media/marda/FOURTB/SDS/Zoo_SDS_paper/field_survey_data/OceanBeach/transects/"
shoreline_dir = r"/media/marda/FOURTB/SDS/Zoo_SDS_paper/field_survey_data/OceanBeach/MSL/geojson/"

shoreline_files = glob.glob(shoreline_dir + "/*.geojson")
...
transect_files = glob.glob(transects_dir + "/*.geojson")
...
for row in shoreline_gdf.iterrows():
    ...
    dates_MSL.append(date_str)


# cross_dist = transect_timeseries(shoreline_gdf,transects_gdf)
...
## all data on transect 6940
# joined_df[joined_df.transect_id.values==6940.]


plt.figure(figsize=(16,16))

plt.subplot(311)
id = np.unique(joined_df.transect_id.values)[0]
...
plt.subplot(312)
id = np.unique(joined_df.transect_id.values)[35]
y = joined_df[joined_df.transect_id.values==id]['cross_distance']
x = joined_df[joined_df.transect_id.values==id]['dates']
plt.plot(x,y,'k-', alpha=0.5)
plt.plot(x,y,'k.')
...
# plt.show()
plt.savefig('OceanBeach_MSL_timeseries_example.png', dpi=300, bbox_inches='tight')
plt.close()


joined_df.to_csv('OceanBeach_MSL_fieldsurvey_shoreline_chainage.csv')

joined_gdf = gpd.GeoDataFrame(
    joined_df, geometry=gpd.points_from_xy(joined_df.x, joined_df.y), crs="EPSG:4326"
)

joined_gdf.to_file("OceanBeach_MSL_fieldsurvey_shoreline_chainage.geojson", driver="GeoJSON") 
...
shoreline_dir = r"/media/marda/FOURTB/SDS/Zoo_SDS_paper/field_survey_data/OceanBeach/MHW/geojson/"
...
# print(f"shoreline files: {shoreline_files}")
shoreline_gdf = combine_geojson_files_with_dates2(shoreline_files)
print(shoreline_gdf.head(2))
print(f"shoreline.crs: {shoreline_gdf.crs}")
...
joined_df.to_csv('OceanBeach_MHW_fieldsurvey_shoreline_chainage.csv')

joined_gdf = gpd.GeoDataFrame(
    joined_df, geometry=gpd.points_from_xy(joined_df.x, joined_df.y), crs="EPSG:4326"
)

joined_gdf.to_file("OceanBeach_MHW_fieldsurvey_shoreline_chainage.geojson", driver="GeoJSON") 


#################### slope
#################################################################################
### filter out MHW dates not present in MSL dataset
joined_gdf_MSL = gpd.read_file("OceanBeach_MSL_fieldsurvey_shoreline_chainage.geojson")
joined_gdf_MHW = gpd.read_file("OceanBeach_MHW_fieldsurvey_shoreline_chainage.geojson")

print(joined_gdf_MSL.head())
print(joined_gdf_MHW.head())


print(len(joined_gdf_MSL))
print(len(joined_gdf_MHW))


msl_dates = np.unique(joined_gdf_MSL['dates']) #217 dates
mhw_dates = np.unique(joined_gdf_MHW['dates']) #219 dates

comb_dates = [d for d in mhw_dates if d in msl_dates] #213
joined_gdf_MHW_2 = joined_gdf_MHW.loc[joined_gdf_MHW["dates"].isin(comb_dates)]
joined_gdf_MSL_2 = joined_gdf_MSL.loc[joined_gdf_MSL["dates"].isin(comb_dates)]


joined_gdf_MSL_2['cross_distance'].values.reshape(len(comb_dates),-1)


MSL_distances_by_time_and_transect = joined_gdf_MSL_2.pivot(index='dates',columns='transect_id', values='cross_distance').values

MHW_distances_by_time_and_transect = joined_gdf_MHW_2.pivot(index='dates',columns='transect_id', values='cross_distance').values


## get horizontal distance
hdist = MHW_distances_by_time_and_transect.values - MSL_distances_by_time_and_transect.values

dz = 0.950976 ## known height between dataums at Ocean Beach in meters (3.12 ft)
...
df = pd.DataFrame(data=slopes,  index=comb_dates, columns=np.unique(joined_gdf_MSL_2['transect_id']))

np.nanmedian(slopes)


df.to_csv('OceanBeach_fieldsurvey_slopes.csv')


plt.figure(figsize=(16,8))

plt.subplot(121)
plt.plot(np.linspace(0,710,71), df.mean(axis=0), label='mean')
plt.plot(np.linspace(0,710,71), df.median(axis=0), label='median')
...
plt.savefig('OceanBeach_slope_space_time.png',dpi=200, bbox_inches='tight')
plt.close()
...
plt.savefig('OceanBeach_Avslope_MonthOfYear.png',dpi=200, bbox_inches='tight')
plt.close()
...
plt.subplot(121)
plt.pcolormesh(np.arange(0,12), np.arange(0,70), np.vstack(MM)[1:,:])
plt.colorbar()
plt.xlabel('Month of Year')
plt.ylabel('Transect ID')
plt.gca().invert_yaxis()
plt.savefig('OceanBeach_spacetime_monthly.png',dpi=200, bbox_inches='tight')
plt.close()
...
# v=[]
# for id in np.unique(joined_df.transect_id.values):
#     v.append(joined_df[joined_df.transect_id.values==id]['cross_distance'])


# for k in joined_df.keys():
#     v.append(joined_df[k])


# plt.pcolormesh(np.arange(len(dates)),np.arange(len(joined_df)), np.vstack(v))
# plt.show()

# df = pd.DataFrame(data=np.vstack(v).T,  index=dates, columns=np.unique(joined_df.transect_id.values)) #cross_dist.keys())
...
# shoreline_dir2 = r"/media/marda/FOURTB/SDS/Zoo_SDS_paper/field_survey_data/Honolulu/SHORELINE_CONTOURS/CONTOURS_MHHW/"
# shoreline_files2 = glob.glob(shoreline_dir2 + "/*mhhw.geojson")
# if not shoreline_files2:
#     raise ValueError(f"No shoreline files found in the directory {shoreline_dir2}")
...
# if shoreline_gdf2.crs == crs:
#     print("CRS match no need to convert shoreline crs")
# else:
#     shoreline_gdf2.to_crs(crs, inplace=True)
# print(f"After using estimate utm crs shoreline.crs: {shoreline_gdf2.crs}")
...
#     date_str = row[1].date.strftime("%d-%m-%Y")
#     geometry = shoreline_gdf2.iloc[row[0]].geometry
#     if geometry is not None:
#         if isinstance(geometry, MultiLineString):
#             for line in geometry.geoms:
#                 shorelines_array = np.array(line.coords)
#                 shorelines2.append(shorelines_array)
...
np.nanmedian(np.vstack(slopes))


df.to_csv('Honolulu_fieldsurvey_slopes.csv')
...
# for row in shoreline_gdf.iterrows():
#     date_str = row[1].date.strftime("%d-%m-%Y")
#     geometry = shoreline_gdf.iloc[row[0]].geometry
#     if geometry is not None:
#         if isinstance(geometry, MultiLineString):
#             for line in geometry.geoms:
#                 shorelines_array = np.array(line.coords)
...
# cross_dist = compute_intersection_QC(shorelines_dict, 
#                             transects_dict,
#                             along_dist = 50, 
#                             min_points = 1,
#                             max_std = 50,
...
# # this scripts assumes all transects are epsg 4326
# transects_dir = r"/media/marda/FOURTB/SDS/Zoo_SDS_paper/field_survey_data/Honolulu/HonolulforZooSDS_paper_analysis/"
# # SLS_T32_transect_epsg4326.geojson

# transect_files = glob(transects_dir + "/*T*4326.geojson")
# print(transect_files)
...
# for file in transect_files:
#     gdf = gpd.read_file(file)#.to_crs(27700, inplace=True)
#     # assign the filename based on the filename
#     id = os.path.basename(file).split("_")[1]
#     gdf["id"] = id
#     # need to concatenate the geodataframes
...
# shoreline_dir = r"/media/marda/FOURTB/SDS/Zoo_SDS_paper/field_survey_data/Honolulu/SHORELINE_CONTOURS/CONTOURS_MSL/"
# shoreline_files = glob(shoreline_dir + "/*msl*.geojson")
...
# transects_gdf.to_crs(32604, inplace=True)
# print(transects_gdf.crs)
# print(f"transects gdf: {transects_gdf.head(2)}")
...
#     if shoreline_gdf.iloc[row[0]].geometry is not None:
...
# transect_files = sorted(glob('forCoastSeg/*transect*.geojson'))


# # read each file into a single geodataframe
# main_gdf = gpd.GeoDataFrame(columns=["geometry", "id"])
...
#     gdf["id"] = id
#     # need to concatenate the geodataframes
#     main_gdf = gpd.GeoDataFrame(pd.concat([main_gdf, gdf], ignore_index=True))

# print(main_gdf.head())
# main_gdf.set_crs(epsg=4326, inplace=True)

# main_gdf.to_file(r"SLS_all_transects_epsg_4326.geojson")
...
# main_gdf = gpd.read_file(r"SLS_all_transects_epsg_4326.geojson")

# transect_dict = get_transect_points_dict(main_gdf)
...
# # ppt = xr.open_dataset("/media/marda/FOURTB/SDS/Zoo_SDS_paper/field_survey_data/Perranporth/PPTdata.mat",engine='netcdf4')


# ds_disk = xr.open_dataset("/media/marda/FOURTB/SDS/Zoo_SDS_paper/field_survey_data/Perranporth/DS01_PPT_Beach3D/DS01_PPT_Beach3D.nc")
...
#     cs=plt.contour(yv,xv,zmat.T,(-99,0,99), colors='k')
#     plt.close()
...
#         df, geometry=gpd.points_from_xy(df.x, df.y), crs="EPSG:27700" #crs="EPSG:27700"
#     )
#     # gdf.to_file(f"{dates[k]}.json", driver="GeoJSON")

#     gdf2 = gdf.groupby(['z'])['geometry'].apply(lambda x: LineString(x.tolist()))
#     gdf2 = gpd.GeoDataFrame(gdf2,  crs="EPSG:27700")
#     gdf2.to_file(f"{d}_line.json", driver="GeoJSON")
