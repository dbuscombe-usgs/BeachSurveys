...
transect_files = glob.glob(transects_dir + "/*T*_epsg*.geojson")
...
shoreline_files = glob.glob(shoreline_dir + "/*msl.geojson")
...
plt.subplot(312)
id = np.unique(joined_df.transect_id.values)[8]
y = joined_df[joined_df.transect_id.values==id]['cross_distance']
x = joined_df[joined_df.transect_id.values==id]['dates']
plt.plot(x,y,'k-', alpha=0.5)
plt.plot(x,y,'k.')
...
# plt.show()
plt.savefig('Waikiki_MSL_timeseries_example.png', dpi=300, bbox_inches='tight')
plt.close()



joined_df.to_csv('Waikiki_MSL_fieldsurvey_shoreline_chainage.csv')

joined_gdf = gpd.GeoDataFrame(
    joined_df, geometry=gpd.points_from_xy(joined_df.x, joined_df.y), crs="EPSG:4326"
)

joined_gdf.to_file("Waikiki_MSL_fieldsurvey_shoreline_chainage.geojson", driver="GeoJSON") 
...
shoreline_dir = r"/media/marda/FOURTB/SDS/Zoo_SDS_paper/field_survey_data/Honolulu/SHORELINE_CONTOURS/CONTOURS_MHHW/"


shoreline_files = glob.glob(shoreline_dir + "/*mhhw.geojson")
if not shoreline_files:
    raise ValueError(f"No shoreline files found in the directory {shoreline_dir}")
...
# plt.show()
plt.savefig('Waikiki_MHW_timeseries_example.png', dpi=300, bbox_inches='tight')
plt.close()



joined_df.to_csv('Waikiki_MHW_fieldsurvey_shoreline_chainage.csv')

joined_gdf = gpd.GeoDataFrame(
    joined_df, geometry=gpd.points_from_xy(joined_df.x, joined_df.y), crs="EPSG:4326"
)

joined_gdf.to_file("Waikiki_MHW_fieldsurvey_shoreline_chainage.geojson", driver="GeoJSON") 



#################################################################################
### SLOPE
joined_gdf_MSL = gpd.read_file("Waikiki_MSL_fieldsurvey_shoreline_chainage.geojson")
joined_gdf_MHW = gpd.read_file("Waikiki_MHW_fieldsurvey_shoreline_chainage.geojson")
...
df.to_csv('Waikiki_fieldsurvey_slopes.csv')


plt.figure(figsize=(16,8))

plt.subplot(121)
plt.plot(np.linspace(0,900,18), df.mean(axis=0), label='mean')
plt.plot(np.linspace(0,900,18), df.median(axis=0), label='median')
...
plt.savefig('Waikiki_slope_space_time.png',dpi=200, bbox_inches='tight')
plt.close()
...
plt.savefig('Waikiki_Avslope_MonthOfYear.png',dpi=200, bbox_inches='tight')
plt.close()
...
plt.subplot(121)
plt.pcolormesh(np.arange(0,12), np.arange(1,18), np.vstack(MM)[1:,:])
plt.colorbar()
plt.xlabel('Month of Year')
plt.ylabel('Transect ID')
plt.gca().invert_yaxis()
plt.savefig('Waikiki_spacetime_monthly.png',dpi=200, bbox_inches='tight')
plt.close()
...
# df.to_csv('Honolulu_fieldsurvey_shoreline_chainage.csv')
